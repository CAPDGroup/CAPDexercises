// Solution  
{
  ...
  // g''(a)= (x_tt*T' + vf)a)*T' + x_t T" + vf_a*T' + x_aa
  ...
  interval T = k/sin(a);
  IVector z = tm(T,s1);
  interval df = vf(T,z)[0]*(-k)*cos(a)/sqr(sin(a)) + ((IMatrix)s1)[0][1]; // f(T(a),a)*T'(a) + x_a(T(a),a)
  ...
  interval d2f = 2.0*dT*(coeffs[2][0]*dT + D[0][1]) + coeffs[1][0]*d2T + 2.*s2(0,1,1); // (x_tt*T' + 2vf_a)*T' + x_t T" + x_aa
  interval N = -df/d2f;
